...
def unfreeze_layers(model, epoch, total_epochs):
    ...
    elif epoch == total_epochs // 2:  # Unfreeze patch embedding after 50%
...
def main():
    ...
    # Differential learning rates - freeze early layers initially
    for param in model.vit.encoder.layers[:-2].parameters():
    ...
    class_dist_val = val_dataset.get_class_distribution()
    num_neg_val = class_dist_val.get(0, 0)  # Healthy cases
    num_pos_val = class_dist_val.get(1, 0)  # Anomalous cases
    print(f"Validation set - Healthy: {num_neg_val}, Anomalous: {num_pos_val}")

    class_dist_test = test_dataset.get_class_distribution()
    num_neg_test = class_dist_test.get(0, 0)  # Healthy cases
    num_pos_test = class_dist_test.get(1, 0)  # Anomalous cases
    print(f"Testing set - Healthy: {num_neg_test}, Anomalous: {num_pos_test}")
...
def create_argparser():
    defaults = dict(
        ...
        training_category=None,  # 'healthy', 'anomalous', 'anomalous_with_findings', or None
